<?php  
    ...
    if(!defined('DK_TPL_PHP'))
        define('DK_TPL_PHP', true);

/**    
 * IRB_Template - PHP templating engine   
...
*/        
    ...
    {  
        $path = $this->tpl_dir . $tpl_name .'.'. $this->tpl_ext;
        $this->start_delim = $this->left_delim . $this->left_delim;
        $this->end_delim   = $this->right_delim . $this->right_delim;
      
        if(!$this->_tpl = @file_get_contents($path)) 
            $this->_debug('template ['. $path .'] does not exist or empty');  
    ...
    protected function _includesPhp($block)   
    {
        ...
        if(!empty($include[2]))
        {
            ...
            {    
                ...
                $block = preg_replace($pattern, '$1 echo "'. $cont .'"$3', $block);
            }
        }
...
*/      
    ...
    {    
        ...
        { 
            ...
            { 
                trigger_error($this->_class .' error: '. $mess); 
            } 
            elseif(!empty($this->exception)) 
            { 
                throw new $this->exception($mess); 
            } 
            else 
            { 
                echo $this->_errhead ? '<div style="color:red; padding:5px; background:#FECBCC">' 
                                              . $this->_class . ' error:</div>' : ''; 
              
                echo '<div style="color:black; padding-left:5px; background:#FECBCC">'  
                     . $mess .'</div>'; 
