...
class Board_Model
{
	...
	protected $region_ = 1672;//Томская область 
...
*/
	public function __construct($table, $num = 1, $city, $public = true)
	{
		...
		$this->tb_city = $tb_city = "city"; // таблица city
		$this->tb_cat = $tb_cat = "category"; // таблица категорий

		$objCity = new GeoCity($this->region_, $this->tb_city, $this->city);
...
* @return mixed
*/
	public function addAdvert($title, $category, $city, $text, $price, $type, $phone, $email, $author, $img, $captcha)
	{
		// $title    = trim($title);
	  ...
	  if(empty($phone))
		...
		if(!isset($_SESSION['userdata']))
		{
		 ...
		 if (empty($_SESSION['img_captcha']) || $_SESSION['img_captcha'] !== $captcha)
		{
			...
			$_SESSION['add_adv']['price'] = $price;
			$_SESSION['add_adv']['type'] = $type;
			$_SESSION['add_adv']['phone'] = $phone;
			$_SESSION['add_adv']['email'] = $email;
			$check_captch = FALSE;
			return FALSE;
		}
		else{
		...
		{
			$_SESSION['msg'] = IRB_LANG_FATAL_ERROR." НЕТ КАПТЧИ";
			return FALSE;
		}
	}
